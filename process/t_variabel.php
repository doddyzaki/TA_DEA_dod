<?php 
	...
	// Mengecek apakah data sudah pernah terdaftar
		$query = 'SELECT COUNT(nama_variabel) AS total FROM tb_variabel WHERE LOWER(nama_variabel)=LOWER("'.$nama_var.'")';
		if (mysqli_query($conn, $query)) {
			...
			if ($data['total'] > 0) {
				...
				$q4 = "INSERT INTO tb_variabel (nama_variabel, jenis_variabel, satuan) VALUES ('$nama_var','$jenis','$satuan')";
				if (mysqli_query($conn, $q4)) {
					...
					$q5 = mysqli_query($conn, 'SELECT * FROM tb_variabel WHERE nama_variabel="'.$nama_var.'" AND jenis_variabel="'.$jenis.'" AND satuan="'.$satuan.'"');
